...
function [breathing, breathing2, heart]=june9CWSourceCode(rd1,beltData)
% Radar Data
...
% Radar Data
% filename1='C:\Users\WEN\Desktop\GUI\Modified GUI\TestingData\AK_WALK_3MIN.csv';
%filename1='C:\Users\Zach\Desktop\CW_MAY25\IN_LY_3MIN.xlsx'
%Data = xlsread(filename1);
%Data = csvread(filename1,1,0);
Data=rd1;
recording_time=180; % Must enter the value of recording time in seconds
%[m1,n11]=size(Data)
%Datachunck=Data(1:20*length(Data)/(180*n11),1:9)
%Data=Data(1:20*length(Data)/(180*n11),1:9)
% Belt Data
%filename1='C:\Users\WEN\Desktop\GUI\Modified GUI\TestingData\AK_WALK_3MIN_BELT.csv';
%filename1='C:\Users\Zach\Desktop\CW_MAY25\IN_LY_3MIN_belt.xlsx'
%belt = xlsread(filename1);
%belt = csvread(filename1,1,1);
belt=beltData;
%% Filter Data
...
div=recording_time/20; % Number of 10 second segments
recording_time=20;
...
[magn index_b]=max(mag_b); % find index with largest power
breathing_ind=index_b+10; % offset by half the window length (to find center)

[M I]=max(abs(z(breathing_ind-25:breathing_ind+25))); % search for peak near estimate
...
[M I]=max(abs(z(2*breathing_ind-25:2*breathing_ind+25))); % search for peak near estimate
...
[magn index]=max(mag_h); % find index with largest power
...
% f1 = 0; % lower frequency bound of chirp transform
% f2 = 4; % upper frequency bound of chirp transform
% w = exp(-j*2*pi*(f2-f1)/(m*fs_b)); % arc in unit circle of z domain is defined by w and a
% a = exp(j*2*pi*f1/fs_b);
